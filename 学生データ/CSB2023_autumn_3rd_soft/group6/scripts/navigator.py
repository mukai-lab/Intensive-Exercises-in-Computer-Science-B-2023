...
class Navigator:
    goal = [[-0.01, -0.05, 0.0, 1.0], [0.01,0.1,0.0,1.0]]
    goal_number = 0
    def __init__(self):
    ...
    def callback_amcl(self, msg):
        ...
        if abs(amcl_x - goal_x) < 0.1 and abs(amcl_y - goal_y) < 0.1 and self.goal_number == 0:
            print("goal!")
            self.goal_number += 1
        if self.goal_number == 0 else self.goal_number -= 1
