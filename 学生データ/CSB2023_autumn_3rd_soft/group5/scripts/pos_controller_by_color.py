...
def cb_set_target_v(data):
        global ROVER_D,target_rover_v, count, count_b
        #目標直進・旋回速度から左右モータの目標回転速度を算出
        if vel_value == 0.1  or count < 13:    #yellow
            print("yellow", count, vel_value)
            count += 1
            data.linear.x = 0
            data.angular.z = 2.0
        elif vel_value == 0  or count_b < 5:
                count_b +=1
                data.linear.x = 0
                data.angular.z = 0
        elif vel_value == 2.0:
...
def cb_color(data):
        global vel_value, count,  count_b
        if data.color_type == 1:    #red
           vel_value = 2.0
        elif data.color_type == 2:   #yellow
           vel_value = 0.1
           count = 0
        elif data.color_type == 3:   #blue
           vel_value =  0
           count_b = 0
        elif data.color_type == 4:   #other
           vel_value = 1.0
        print(data.color_type)
