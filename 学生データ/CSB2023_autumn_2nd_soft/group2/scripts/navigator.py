...
class Navigator:
    goal = [[0.2, 0.2, 0.0, 1.0],[0.2,-0.2,0.0,1.0],[-0.2,-0.2,0.0,1.0]]
    goal_number = 0
    def __init__(self):
    ...
    def callback_amcl(self, msg):
        if abs(msg.pose.pose.position.x - self.goal[self.goal_number][0]) < 0.1 and abs(msg.pose.pose.position.y - self.goal[self.goal_number][1]) < 0.1:
                print(f"""goal {self.goal_number} ({msg.pose.pose.position.x},{msg.pose.pose.position.y})
                odom(({odom_x}, {odom_y}) theta: {odom_theta}""")
                self.goal_number = self.goal_number + 1
                if self.goal_number > 2:
                    self.goal_number = 0
                self.setGoal(self.goal[self.goal_number][0], self.goal[self.goal_number][1], self.goal[self.goal_number][2], self.goal[self.goal_number][3])
