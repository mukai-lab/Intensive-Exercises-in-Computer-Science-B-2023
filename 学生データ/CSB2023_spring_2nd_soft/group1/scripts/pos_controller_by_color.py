...
def cb_set_target_v(data):
        global ROVER_D,target_rover_v,count
        #目標直進・旋回速度から左右モータの目標回転速度を算出
        #target_rover_v[1] = (data.linear.x + ROVER_D * data.angular.z) * vel_value
        #target_rover_v[0] = -1.0 * (data.linear.x - ROVER_D * data.angular.z) * vel_value
        #time_sta = time.time()
        if vel_value == 0:
                print("red")
                #time_sta = time.time()
                #time.sleep(1)
                #time_end = time.time()
                target_rover_v[1] = 0.0
                target_rover_v[0] = 0.0
        elif vel_value == 1:
                print("yellow")
                time_sta = time.time()
                time.sleep(1)
                time_end = time.time()
                target_rover_v[1] = 0.25
                target_rover_v[0] = 0.0
        elif vel_value == 2:
                print("blue")
                target_rover_v[1] = 0.04
                target_rover_v[0] = -0.04        
        else:
                print("hosei")
                target_rover_v[1] = 0.1
                target_rover_v[0] = 0.0
        #time_end = time.time()
