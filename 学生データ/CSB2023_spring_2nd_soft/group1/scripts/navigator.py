...
class Navigator:
    ...
    def callback_amcl(self, msg):
        ...
        print(x ,y)
        if(x >= self.goal[self.goal_number][0] - 0.1 and x <= self.goal[self.goal_number][0] + 0.1):
            if(y >= self.goal[self.goal_number][1] - 0.1 and y <= self.goal[self.goal_number][1] + 0.1):
