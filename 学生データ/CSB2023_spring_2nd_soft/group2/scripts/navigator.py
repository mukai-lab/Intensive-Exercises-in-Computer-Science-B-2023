...
class Navigator:
    goal = [[-0.2, -0.2, 0.5, 1.0],[0.1,0.1,0.5,1.0],[-0.1,0.1,0.5,1.0],[0.1,0.1,0.5,1.0],[0.1,-0.1,0.5,1.0]]
    goal_number = 0
    def __init__(self):
    ...
    def callback_amcl(self, msg):
        if((self.goal[self.goal_number][0]-0.1) < msg.pose.pose.position.x and msg.pose.pose.position.x < (self.goal[self.goal_number][0]+0.1)):
            if((self.goal[self.goal_number][1] - 0.1) < msg.pose.pose.position.y and  msg.pose.pose.position.y < (self.goal[self.goal_number][1] + 0.1)):
                print(msg.pose.pose.position.x)
                print(msg.pose.pose.position.y)
                print(self.goal[self.goal_number][0])
                print(self.goal[self.goal_number][1])
