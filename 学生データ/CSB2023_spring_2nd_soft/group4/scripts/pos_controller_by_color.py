...
def cb_set_target_v(data):
        ...
        if Flag == 4:
            ...
            target_rover_v[0] = 0
            return
        if (vel_value * data.linear.x + get_not(vel_value)) == 0:
            Flag += 1

            
        print(Flag)
        #目標直進・旋回速度から左右モータの目標回転速度を算出
        target_rover_v[1] = 0.1*(vel_value * data.linear.x + get_not(vel_value)) 
        target_rover_v[0] = -0.1*(vel_value * data.linear.x - get_not(vel_value))
        
def cb_color(data):
        global vel_value
        global Flag
        if data.color_type == Flag:
            vel_value = 1
        else:
            vel_value = 0
        #print(vel_value)
