...
class Navigator:
    goal = [[-0.2, 0.2, 0.0, 1.0], [0.2, -0.2, 0.0, 1.0], [-0.21, 0.28, 0.0, 1.0], [0.28, -0.21, 0.0, 1.0]]
    goal_number = 0
    def __init__(self):
    ...
    def callback_amcl(self, msg):
        x_length = self.goal[self.goal_number][0] - msg.pose.pose.position.x
        y_length = self.goal[self.goal_number][1] - msg.pose.pose.position.y
        gosa = 0.10
        print(x_length, y_length)
        if -gosa < x_length < gosa:
            if -gosa < y_length < gosa:
                print("Goal!!!!!!")
