...
class Navigator:
    goal = [[0.15, 0.15, 0.0, 1.0],[0.15, -0.15, 0.0, 1.0],[-0.15, -0.15, 0.0, 1.0],
            [-0.15, 0.15, 0.0, 1.0]]
    goal_number = 0
    ...
    def callback_amcl(self, msg):
        global odom_x, odom_y
        if msg.pose.pose.position.x > self.goal[self.goal_number][0] - 0.1 and msg.pose.pose.position.x < self.goal[self.goal_number][0] + 0.1 and msg.pose.pose.position.y > self.goal[self.goal_number][1] - 0.1 and msg.pose.pose.position.y < self.goal[self.goal_number][1] + 0.1:
            print("AMCL")
