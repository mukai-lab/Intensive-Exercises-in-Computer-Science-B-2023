...
class ColorExtract(object):
    ...
    def callback(self, data):
        ...
        yellow_area, yellow_image = self.get_colored_area(
            cv_image, np.array([20,50,50]), np.array([50,255,255]))
        ...
        if self._color.color_type == 0:
            if red_area > 30000:
                self._color.color_type = 1
            if blue_area > 30000:
                self._color.color_type = 2
        
        if self._color.color_type == 1 and red_area < 30000:
            self._color.color_type = 3
        if self._color.color_type == 2 and blue_area < 30000:
            self._color.color_type = 4

        if self._color.color_type == 3 and red_area > 30000:
            self._color.color_type = 5
        if self._color.color_type == 4 and blue_area > 30000:
            self._color.color_type = 5
        self._color_detector_pub.publish(self._color)


            
        '''if self._color.color_type == 1 or self._color.color_type == 4:
            self._color.color_type = 4
        elif yellow_area > 50000:
            self._color.color_type = 2
        elif blue_area > 50000:
            self._color.color_type = 3
        elif red_area > 10000:
            self._color.color_type = 1
        elif yellow_area < 50000 and self._color.color_type == 2:
            self._color.color_type = 5
        elif blue_area < 50000 and self._color.color_type == 3:
            self._color.color_type = 6
        else:
            self._color.color_type = 0'''
